...
- Qwen3.6-27B: max reliable ctx on the 24 GB card is **48K with q8_0 KV** (UD-Q4_K_XL). 56K+ needs a
  ...
  Full Forge 30-scenario eval 2026-08-07 (v0.16.1): **149/150 (99.3%) completion, 141/150 (94%)
  accuracy** — data_gap_recovery_extended the only weak spot (40%/0% acc).
...
```bash
pytest tests/unit/ -q              # All 687 tests
pytest tests/unit/ -q -k "loop"    # Specific rule
```

All 687 tests must pass before committing.
...
```bash
source .venv/bin/activate
pytest tests/unit/ -q          # All 687 tests MUST pass
```
